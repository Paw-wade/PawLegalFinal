        ...
        <div className="bg-white rounded-xl shadow-lg p-8 border border-gray-200">
          ...
          <section className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {recap.client.inscritDepuis && (
                <div>
                  <p className="text-sm text-gray-500 mb-1">Inscrit depuis</p>
                  <p className="font-semibold">{formatDate(recap.client.inscritDepuis)}</p>
                </div>
              )}
            </div>
          </section>
          ...
          {/* Équipe */}
          <section className="mb-8">
            <h2 className="text-2xl font-bold text-foreground mb-4">Équipe de Traitement</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {recap.equipe.createur && (
                <div>
                  <p className="text-sm text-gray-500 mb-1">Créateur</p>
                  <p className="font-semibold">{recap.equipe.createur.nom}</p>
                  <p className="text-sm text-gray-500">{recap.equipe.createur.email}</p>
                </div>
              )}
              {recap.equipe.chefEquipe && (
                <div>
                  <p className="text-sm text-gray-500 mb-1">Chef d'équipe</p>
                  <p className="font-semibold">{recap.equipe.chefEquipe.nom}</p>
                  <p className="text-sm text-gray-500">{recap.equipe.chefEquipe.email}</p>
                </div>
              )}
              {recap.equipe.membres && recap.equipe.membres.length > 0 && (
                <div className="md:col-span-2">
                  <p className="text-sm text-gray-500 mb-1">Membres de l'équipe</p>
                  <div className="flex flex-wrap gap-2">
                    {recap.equipe.membres.map((m: any, idx: number) => (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600 mt-2">
                          ...
                          <span>Assigné à: {task.assigneA}</span>
                          {task.dateEcheance && <span>Échéance: {formatDate(task.dateEcheance)}</span>}
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          {/* Rendez-vous */}
          {recap.rendezVous.total > 0 && (
            <section className="mb-8">
              <h2 className="text-2xl font-bold text-foreground mb-4 flex items-center gap-2">
                <Calendar className="w-6 h-6 text-primary" />
                Rendez-vous ({recap.rendezVous.total})
              </h2>
              <div className="mb-4 flex items-center gap-4">
                <span className="px-3 py-1 bg-gray-100 text-gray-800 rounded-lg text-sm">
                  Passés: {recap.rendezVous.passes}
                </span>
                <span className="px-3 py-1 bg-blue-100 text-blue-800 rounded-lg text-sm">
                  À venir: {recap.rendezVous.aVenir}
                </span>
              </div>
              <div className="space-y-3">
                {recap.rendezVous.liste.map((rv: any, idx: number) => (
                  <div key={idx} className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <p className="font-semibold">{formatDate(rv.date)} à {rv.heure}</p>
                        <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600 mt-2">
                          <span>Statut: {rv.statut}</span>
                          {rv.type && <span>Type: {rv.type}</span>}
                        </div>
                        {rv.notes && (
                          <p className="text-sm text-gray-500 mt-2">{rv.notes}</p>
                        )}
                      </div>
                    </div>
                  </div>
          ...
          {recap.transmissions && recap.transmissions.length > 0 && (
            <section className="mb-8">
              <h2 className="text-2xl font-bold text-foreground mb-4">Transmissions aux Partenaires</h2>
              <div className="space-y-3">
                  ...
                  trans.partenaire && (
                    <div key={idx} className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600 mt-2">
                        ...
                        <span>Par: {trans.transmisPar}</span>
                      ...
                      {trans.notes && (
                        <p className="text-sm text-gray-500 mt-2">{trans.notes}</p>
                      )}
                    </div>
                  )
                ))}
          ...
          {recap.dossier.notes && (
            <section className="mb-8">
              <h2 className="text-2xl font-bold text-foreground mb-4">Notes Internes</h2>
              <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                <p className="text-gray-700 whitespace-pre-wrap">{recap.dossier.notes}</p>
              </div>
            </section>
          )}
        </div>
