...
export default function PlatformCabinetsPage() {
  ...
  const allowed = canAccessPlatformConsole(user?.role, user?.email);
  ...
  const handleSuspend = async (slug: string) => {
    ...
    try {
      ...
      setError(e?.response?.data?.message || 'Suspension impossible');
    }
  };
  ...
  const handleProvision = async () => {
    ...
    try {
      ...
      setProvisionMsg(e?.response?.data?.message || 'Échec');
    }
  };
    ...
    <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            ...
            Console Ada Papers — Cabinets
          </h1>
          <p className="text-sm text-gray-600 mt-1">
            Gestion multi-tenant : organisations, provisioning et checklist DNS / Vercel.
          </p>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-4 h-fit">
          ...
          <Link href="/admin" className="block text-center text-xs text-gray-500 hover:text-primary">
            ← Retour admin cabinet
          </Link>
        </div>
      </div>
    </div>
